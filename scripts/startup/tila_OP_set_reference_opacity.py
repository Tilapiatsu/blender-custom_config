...
class TILA_OBJECT_PT_RegisterOpacityControl(Panel):
    ...
    def draw(self, context):
        ...
        if context.object.name not in get_registered_references_name(context):
            op = layout.operator("object.tila_register_opacity_control_object")
            op.reference_object_name = context.object.name
        else:
            op = layout.operator("object.tila_unregister_opacity_control_object")
            op.reference_object_name = context.object.name
...
class TILA_RegisterOpacityControlObjectOperator(bpy.types.Operator):
    ...
    reference_object_name: bpy.props.StringProperty(name="Reference Object Name")
    ...
    def execute(self, context):
        ...
        if self.reference_object_name not in bpy.data.objects:
            self.report({"ERROR"}, f"Invalid object selected {self.reference_object_name}")
            return {"CANCELLED"}

        ref = context.scene.tila_opacity_controled_references.add()
        ref.reference = bpy.data.objects.get(self.reference_object_name)
...
class TILA_UnRegisterOpacityControlObjectOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if self.reference_object_name not in get_registered_references_name(context):
            self.report({"ERROR"}, f"Object not registered properly : {self.reference_object_name}")
            return {"CANCELLED"}

        index = find_reference_in_collection(context, bpy.data.objects[self.reference_object_name])
        if index == -1:
            self.report({"ERROR"}, f"Object not registered properly : {self.reference_object_name}")
            return {"CANCELLED"}
...
class TILA_SetRefImagesOpacityOperator(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        self.snap_step = math.radians(round(math.degrees(45)))
