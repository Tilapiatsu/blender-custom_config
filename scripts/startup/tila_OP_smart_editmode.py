...
class TILA_smart_editmode(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        elif bpy.context.mode in ['PAINT_GPENCIL', 'VERTEX_GPENCIL', 'WEIGHT_GPENCIL']:
            if self.alt_mode:
                bpy.ops.object.mode_set(mode='OBJECT')
            else:
                if self.mode == 0 and not bpy.context.scene.tool_settings.use_gpencil_vertex_select_mask_point:                   
                    ...
                    bpy.context.scene.tool_settings.use_gpencil_vertex_select_mask_stroke = False
                    bpy.context.scene.tool_settings.use_gpencil_vertex_select_mask_segment = False

        elif bpy.context.mode in ['SCULPT_GPENCIL']:
            if self.alt_mode:
                bpy.ops.object.mode_set(mode='OBJECT')
            else:
                if self.mode == 0 and not bpy.context.scene.tool_settings.use_gpencil_select_mask_point:                   
                ...
                elif self.mode == 2 and not bpy.context.scene.tool_settings.use_gpencil_select_mask_segment:
                    ...
                    bpy.context.scene.tool_settings.use_gpencil_select_mask_stroke = False
                    bpy.context.scene.tool_settings.use_gpencil_select_mask_segment = False


        elif bpy.context.mode in ['PAINT_WEIGHT', 'PAINT_VERTEX', 'PAINT_TEXTURE']:
            if self.alt_mode:
                bpy.ops.object.mode_set(mode='OBJECT')
            else:
