...
class TILA_ToggleSymOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if context.object.type in ['CURVES']:
            if self.axis == 'X':
                ...
                context.object.data.use_mirror_y = not context.object.data.use_mirror_y
                self.report({'INFO'}, 'Y Symetry {}'.format('ON' if context.object.use_mesh_mirror_y else 'OFF'))
            elif self.axis == 'Z':
                context.object.data.use_mirror_z = not context.object.data.use_mirror_z
