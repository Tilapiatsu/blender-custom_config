...
class TILA_SculptMaskFaceSet(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        if self.mode == "MASK":
            # print("Masking Faceset")
            bpy.ops.paint.hide_show_all(action="SHOW")
            bpy.ops.paint.mask_flood_fill(mode="VALUE", value=0.0)
            bpy.ops.sculpt.face_set_change_visibility("INVOKE_DEFAULT", mode="TOGGLE", active_face_set=0)
            bpy.ops.paint.mask_flood_fill(mode="INVERT")
            bpy.ops.paint.hide_show_all(action="SHOW")
            bpy.ops.paint.mask_flood_fill(mode="INVERT")
        elif self.mode == "TOGGLE":
            # print("Toggle Faceset")
            bpy.ops.paint.hide_show_all(action="SHOW")
            bpy.ops.paint.mask_flood_fill(mode="INVERT")
...
classes = (TILA_SculptMaskFaceSet,)
