...
def get_selected_objects():
    ...
    objs = [o for o in selected_objects if o.type in compatible_type]
    return objs

class TILA_SeparateAndSelect(bpy.types.Operator):
    bl_idname = "view3d.separate_and_select"        # unique identifier for buttons and menu items to reference.
    ...
    def separate(self, obj, type:str='SELECTED')-> None:
        if obj.type == 'MESH':
            ...
            bpy.ops.grease_pencil.separate(mode=type)

    def select_all(self, obj, action:str='SELECT')-> None:
        if obj.type == 'MESH':
            bpy.ops.mesh.select_all('INVOKE_DEFAULT', action=action)
        elif obj.type == 'CURVE':
            bpy.ops.curve.select_all('INVOKE_DEFAULT', action=action)
        elif obj.type == 'CURVES':
            bpy.ops.curves.select_all('INVOKE_DEFAULT', action=action)
        elif obj.type == 'GPENCIL':
            bpy.ops.grease_pencil.select_all('INVOKE_DEFAULT', action=action)

    def separate_selected(self, context):
        self.separate(obj=self.current_object_to_proceed, type='SELECTED')
        ...
        for i,o in enumerate(extracted_objects):
            o.select_set(True)
            if i == 0:
        ...
        bpy.ops.object.editmode_toggle()
        self.select_all(obj=self.current_object_to_proceed)
    ...
    def execute(self, context):
        ...
        self.current_object_to_proceed = self.objects_to_proceed.pop()
        if self.current_object_to_proceed.type == 'MESH' and self.by_loose_parts:
            self.separated = False
