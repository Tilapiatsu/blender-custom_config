...
import bpy
import blf
from .ke_utils import get_selected, getset_transform, restore_transform, get_layer_collection, get_distance, average_vector
...
class VIEW3D_OT_ke_radialarray(bpy.types.Operator):
    ...
    def modal(self, context, event):
        ...
        if event.type in {'LEFTMOUSE', 'RET', 'SPACE'}:
            ...
            bpy.types.SpaceView3D.draw_handler_remove(self._handle, 'WINDOW')
            context.area.tag_redraw()
            # restore_transform(self.og) # maybe not
            return {'FINISHED'}
