...
PREFERENCE_TABS = [("GENERAL", "General", ""), ("ADDONS", "Addons", ""), ("ABOUT", "About", "")]
...
class TILA_Config_Preferences(bpy.types.AddonPreferences):
    ...
    def draw_progress(self, box, wm):
        ...
        row.template_list(
            "TILA_UL_Config_status_list",
            "",
            wm,
            "tila_config_status_list",
            wm,
        ...
        row.template_list(
            "TILA_UL_Config_log_list", "", wm, "tila_config_log_list", wm, "tila_config_log_list_idx", rows=rows
        )
    ...
    def draw_addon_item(self, context, layout, item, installed: bool):
        ...
        if panel:
            ...
            if item.keymaps:
                ...
                op = col_b.operator(
                    "tila.config_register_keymaps", text=f"Apply Keymaps for {item.name}", icon="EVENT_K"
                )
                op.name = item.name
            ...
            if len(item.paths):
                ...
                col_b.operator(
                    "tila.config_link_addon_list", text=f"Link {item.name}", icon="LINK_BLEND"
                ).name = item.name
                if installed:
                    for i in range(len(item.paths)):
                        ...
                        if os.path.exists(item_path):
                            ...
                            op = col_b.operator("tila.config_open_folder", text="Open Path")
                            op.path = item_path
