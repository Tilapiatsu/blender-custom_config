...
class TILA_Config_Settings_Base:
    ...
    def __init__(self):
        self.log_progress = log_list(
            bpy.context.window_manager.tila_config_log_list, "tila_config_log_list_idx"
        )
                ...
                if test_addon_exists:
                    if self.addon_name not in bpy.context.preferences.addons:
                        self.log_progress.start(
                            f"Addon {self.addon_name} is not enable: skipping"
                        )
                        return
    ...
    def set_setting(self, setting_root: bpy.types.bpy_struct, setting_name: str, value):
        if rgetattr(setting_root.preferences, setting_name, None) is None:
            self.log_progress.error(
                f'Setting "{setting_name}" not found in {self.addon_name} addon'
            )
            return
