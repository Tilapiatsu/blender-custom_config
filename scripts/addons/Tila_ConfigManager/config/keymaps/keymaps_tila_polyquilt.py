    ...
    def set_keymaps(self):
        if self.kmi_init(
            ...
            self.kmi_set_replace(
                ...
                "PRESS",
                shift=True,
                properties={"name": "mesh_tool.poly_quilt"},
                disable_double=True,
            )

        ###### 3D View Tool: Edit Mesh, PolyQuilt
        if self.kmi_init(
            name="3D View Tool: Edit Mesh, PolyQuilt",
            space_type="VIEW_3D",
            region_type="WINDOW",
            addon=False,
            restore_to_default=False,
            ...
            self.kmi_set_replace(
                ...
                "PRESS",
                shift=True,
                properties={"tool_mode": "BRUSH", "brush_type": "SMOOTH"},
            )
            self.kmi_set_replace(
                "mesh.poly_quilt",
                self.k_cursor,
                "PRESS",
                shift=True,
                properties={"tool_mode": "BRUSH", "brush_type": "MOVE"},
                disable_double=True,
            )
            self.kmi_set_replace(
                "mesh.poly_quilt",
