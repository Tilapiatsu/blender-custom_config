...
def add_driver(ob, world):
    ...
    for i, axis in enumerate('xyz'):
        ...
        path = f'node_tree.nodes["Easy HDRI"].inputs["Rotation"].default_value[{i}]'
...
def reset_default_settings(context, reset_type):
    ...
    if reset_type == 'ALL' or reset_type == 'MAIN':
        node.inputs['Sun Strength'].default_value = 0.0
        node.inputs['Sky Strength'].default_value = 1.0
    ...
    if reset_type == 'ALL' or reset_type == 'BG':
        ...
        node.inputs[4].default_value = (0.0, 0.0, 0.0, 1.0)
        node.inputs[5].default_value = 1.0
        node.inputs[6].default_value = (0.0, 0.0, 20.0)
    ...
    if reset_type == 'ALL' or reset_type == 'COL':
        node.inputs[11].default_value = (0.5, 0.5, 0.5, 1.0)
        node.inputs[12].default_value = 0.0
        node.inputs[13].default_value = 1.0
        node.inputs[14].default_value = 1.0
...
def draw_callback_px(self, context):
    ...
    angle = node.inputs['Rotation'].default_value[2]
...
class EASYHDRI_OT_rotate_hdri(Operator):
    ...
    def invoke(self, context, event):
        scn = context.scene
        if check_world_nodes(scn, check_users=False) != 'OK':
            self.report({'WARNING'}, "Enable to find EasyHDRI Node Group")
            return {'CANCELLED'}

        node = scn.world.node_tree.nodes.get('Easy HDRI')
        self.hdri_rot = node.inputs['Rotation'].default_value
...
class EASYHDRI_PT_main(Panel):
    ...
    def draw(self, context):
        ...
        check = check_world_nodes(scn, check_users=False)
...
class EASYHDRI_PT_main_settings(Panel):
    ...
    def poll(self, context):
        return check_world_nodes(context.scene, check_users=False) == 'OK'
    ...
    def draw(self, context):
        ...
        row.separator()
        col = row.column()
        col.prop(node.inputs['Sun Strength'], "default_value", text = 'Sun Strength')
        col.prop(node.inputs['Sky Strength'], "default_value", text = 'Sky Strength')
        col.prop(nodes['Environment'], "projection", text = '')
        index = -1 if context.scene.easy_hdri_props.all_axis else 2
        col.prop(node.inputs["Rotation"], "default_value", text = "Rotation", index=index)
...
class EASYHDRI_PT_background(Panel):
    ...
    def draw(self, context):
        ...
        if props.easyhdr_bg_display == 'Solid':
            col.prop(node.inputs[4], "default_value", text = "")
            col.prop(node.inputs[5], "default_value", text='Strength')
            ...
            if props.ground_projection:
                col.prop(node.inputs[6], "default_value",text='Size', index=2)
                col.prop(node.inputs[7], "default_value",text='Horizon', index=2)
                col.prop(nodes['Environment Projected'], "projection", text = '')
                col.prop(node.inputs[2], "default_value", text='Strength')
# Color
class EASYHDRI_PT_color(Panel):
    ...
    def draw(self, context):
        ...
        col.prop(node.inputs[11], "default_value", text = "Tint")
        col.prop(node.inputs[12], "default_value", text = "Factor")
        col.separator()
        col.prop(node.inputs[13], "default_value", text = "Gamma")
        col.separator()
        col.prop(node.inputs[14], "default_value", text = "Saturation")
