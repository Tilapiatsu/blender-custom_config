...
import bmesh
import bpy
from bpy.props import EnumProperty, IntVectorProperty, BoolProperty
...
def reselect(sel_mode, sel_verts, sel_edges, sel_faces, bm, og_ae, expand):
    if sel_mode[1]:
...
def process(context, m_axis_mode, axis, axis_sign, obj, ae, sel_mode, mousepos):
    ...
    mouse_wpos = region_2d_to_location_3d(region, region_data, mousepos, ae_wco)
    ...
    return ae, me, sel_verts, sel_edges, sel_faces, axis, sign, axis_sign, m_axis_sign, m_axis
...
class KeMouseSideofActive(Operator):
    ...
    def execute(self, context):
        ...
        ae, me, sel_verts, sel_edges, sel_faces, axis, sign, axis_sign, m_axis_sign, m_axis = process(
            context, m_axis_mode,
            self.axis, self.axis_sign,
            obj, ae, sel_mode,
            self.mouse_pos
        )
...
class KeMouseSelectMirror(Operator):
    bl_idname = "mesh.ke_mouse_select_mirror"
    bl_label = "Mouse Select Mirror"
    bl_description = "Select Mirror, but using active vert, edge or face with mouse position" \
                     "to calculate which side of the active element to mirror select"
    bl_options = {'REGISTER', 'UNDO'}
    ...
    mouse_pos : IntVectorProperty(size=2)
    expand : BoolProperty(default=False, name="Extend", description="Extend selection / New selection")
    ...
    def draw(self, context):
        ...
        col = layout.column()
        col.use_property_split = True
        col.prop(self, "axis", expand=True)
        col.prop(self, "expand", toggle=True)
        col.separator(factor=1)
        col = layout.column()
        col.active = False
    ...
    def execute(self, context):
        ...
        # bmesh.update_edit_mesh(me)
        bpy.ops.mesh.select_mirror("INVOKE_DEFAULT", axis={axis}, extend=self.expand)
