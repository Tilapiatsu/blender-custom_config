...
class OBJECT_OT_snap_curve(Operator):
    ...
    def execute(self, context):
        ...
        bpy.context.scene.cursor.location = headloc


            #add target object
        bpy.context.scene.cursor.location = bpy.data.objects[obj.name].location + context.active_pose_bone.tail
        ...
        obj.select_set(True)
        context.view_layer.objects.active = obj
        bpy.context.scene.cursor.location = co2
        ...
        bpy.context.scene.cursor.location = co1
        ...
        bpy.context.scene.cursor.location = inxW
        ...
        bpy.context.scene.cursor.location = outyW
        
        editbone = bpy.data.armatures[obj.name].bones[pb.name]
        
        inxL2 = Vector((inxL[0],inxL[2]))
        outyL2 = Vector((outyL[0],outyL[2]))
        pbcurvei = Vector((pb.bbone_curveinx,pb.bbone_curveinz))
        pbcurveo = Vector((pb.bbone_curveoutx,pb.bbone_curveoutz))
        ...
        pb.bbone_curveinz = rpbcurvei[1] - (editbone.bbone_curveinz*inf)
        pb.bbone_curveoutx = rpbcurveo[0] - (editbone.bbone_curveoutx*inf)
        pb.bbone_curveoutz = rpbcurveo[1] - (editbone.bbone_curveoutz*inf)
...
class UIListPanelExample(Panel):
    ...
    def draw (self, context):
            ...
            row = layout.row(align=True)
            row.prop(snapProperties, "lockTail",  icon='DECORATE_LOCKED', text="Tail")
            row.prop(snapProperties, "lockHead",  icon='DECORATE_LOCKED', text="Head")
