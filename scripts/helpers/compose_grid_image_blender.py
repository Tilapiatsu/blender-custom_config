...
def get_max_row_height(images_to_process, max_row_size):
    ...
    max_crops = []
    nb_images = len(images_to_process)
...
def get_images_to_process(source_folder):
    ...
    for i, e in enumerate(images_to_process):
        if normalize_size[0]:
            normalized_images_to_process.append(
                (e[0], (e[1][0], e[1][1], max_width, math.ceil(e[1][3] * max_width / e[1][2])))
            )
        elif normalize_size[1]:
            normalized_images_to_process.append(
                (e[0], (e[1][0], e[1][1], math.ceil(e[1][2] * max_height / e[1][3]), max_height))
            )
        else:
...
def get_image_size(images_to_process, max_row_size, max_row_height, max_collumn_width):
    ...
    for i in range(len(images_to_process)):
        ...
        if i < max_row_size:
            image_size = (
                math.floor((image_size[0] - width_crop[0] + width_crop[1]) * (1 + crop_overscan)),
                image_size[1],
            )
...
for i, current_image_to_process in enumerate(images_to_process):
    ...
    with Image.open(image_path) as image:
        ...
        if image_number % max_row_size == 0:
            paste_position = (
                paste_position[0],  # Left 0
                paste_position[1] + math.floor(image_size[1] / max_row_size * crop_overscan * 8),  # Top 1
                paste_position[2],  # Right 2
                paste_position[1] + cropped.size[1] + math.floor(image_size[1] / max_row_size * crop_overscan * 8),
            )  # Bottom 3

        paste_position = (
            paste_position[2] + math.floor(image_size[0] / max_row_size * crop_overscan * 2),  # Left 0
            paste_position[1],  # Top 1
            paste_position[2]
            + cropped.size[0]
            + math.floor(image_size[0] / max_row_size * crop_overscan * 2),  # Right 2
            paste_position[3],
        )  # Bottom 3
